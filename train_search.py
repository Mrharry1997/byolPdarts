...
parser.add_argument('--tmp_data_dir', type=str, default='/home/PJLAB/zhazhenzhou/Dataset/', help='temp data dir')
...
parser.add_argument('--img_size', default=32, type=int, help='input image size, default: 32 (CIFAR10)')
    ...
    for sp in range(len(num_to_keep)):
        ...
        for epoch in range(epochs):
            ...
            if epoch < eps_no_arch:
                ...
                train_obj = train(train_queue, valid_queue, model, network_params, optimizer, optimizer_a, train_arch=False)
            else:
                model.module.p = float(drop_rate[sp]) * np.exp(-(epoch - eps_no_arch) * scale_factor) 
                model.module.update_p()                
                train_obj = train(train_queue, valid_queue, model, network_params, optimizer, optimizer_a, train_arch=True)
...
def train(train_queue, valid_queue, model, network_params, optimizer, optimizer_a, train_arch=True):
    objs = utils.AvgrageMeter()
    
    for step, (input, _) in enumerate(train_queue):
        model.train()
        n = input.size(0)
        input = input.cuda()
        ...
        if train_arch:
            ...
            try:
                input_search, _ = next(valid_queue_iter)
            except:
                valid_queue_iter = iter(valid_queue)
                input_search, _ = next(valid_queue_iter)
            input_search = input_search.cuda()
            optimizer_a.zero_grad()
            loss_a = model(input_search)
            loss_a.backward()
